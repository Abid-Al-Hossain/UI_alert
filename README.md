# UI Foundry Alert Studio

Standalone visual Alert generator built with Next.js, React, and TypeScript.

Standalone Alert component studio for building native, preview-honest, React-exportable UI patterns that match the UI Foundry product line.
...
- designers building Alert UI quickly
- developers who need editable React starting points
- marketplace buyers who want a self-contained Alert generator
...
The editor is focused on controls that are native to Alert and useful in real product work:
- title, description, status, icon, action, dismiss, and compact/full layout
- info, success, warning, error, neutral, banner, inline, and card alert variants
- surface, border, radius, shadow, typography, spacing, and motion-safe entrance
- role/status/alert semantics and dismiss labeling
- React export for alert component

The goal is maximum useful depth without adding unrelated controls that would make the component stop reading as Alert.
...
This repo is intended to publish as Abid-Al-Hossain/UI_alert.
